...
def _pad_to_block(t: torch.Tensor, target_len: int, dim: int = 2) -> torch.Tensor:
    ...
    if t.size(dim) >= target_len:
        ...
        return t
    pad = torch.zeros(*t.shape[:dim], target_len - t.size(dim), *t.shape[dim+1:], dtype=t.dtype, device=t.device)
...
class RingAttentionHelper:
    ...
    def __init__(self, attn_module: nn.Module, strategy: Any, llama_block: nn.Module,
        ...
        self.attn, self.ff, self.ff_norm, self.strategy, self.use_cache, self.llama_block = attn_module, ff, ff_norm, strategy, use_cache, llama_block
    ...
    def forward_full(self, q, k, v, mask, valid_len, x_block, x_norm_block, q_start_global):
        ...
        num, denom = self._compute_sums_pass(q, k, v, mask, q_start_global, valid_len, max_score)
    ...
    # _compute_max_score_pass, _compute_sums_pass, _compute_attention_scores, _update_max_score,
    # _update_totals, _ring_shift_tensor are unchanged. Concise pass ends here for clarity.
    ...
    def _compute_sums_pass(self, q_compute, k_compute, v_compute, mask_global, q_start_global, valid_len_local, max_score):
        B, H, T, D = q_compute.shape
        ...
        for i in range(self.world_size):
            ...
            if k_len > 0:
                s = self._compute_attention_scores(q_compute, k_blk[:, :, :k_len], q_idx, k_idx, m)
                e = torch.exp((s - max_score).clamp(EXP_CLAMP_MIN, EXP_CLAMP_MAX))
    ...
    def _compute_attention_scores(self, q, k, q_idx, k_idx, mask=None, apply_mask=True, keep_causal=True):
        ...
        if apply_mask:
            if mask is not None:
                assert mask.shape in [(B, H, Tq, Tk), (B, 1, Tq, Tk)]
    ...
    def _update_max_score(self, scores, current_max):
        ...
        block_max = scores.masked_fill(scores == -torch.inf, min_val).amax(-1, keepdim=True)
    ...
    def _update_totals(self, scores, v, max_score, numerator, denominator):
        ...
        numerator += torch.einsum("bhqk,bhkd->bhqd", exp, v)
        denominator += exp.sum(-1, keepdim=True)
