...
class RingAttentionStrategy(DistributedStrategy):
    ...
    def _pad_to_block_size(
        self, x: Tensor, dim: int = 1
    ) -> Tensor:
        length = x.size(dim)
        if length == self.block_size:
            return x
        assert length < self.block_size, (
            f"Rank {self.rank}: length {length} must be < block_size {self.block_size}"
        )
    ...
    def shard_input(
        ...
        if self.world_size == 1:
            ...
            if seq_len < self.block_size:
                return self._pad_to_block_size(x, dim=1)
            return x
        start = self.rank * self.block_size
        end = min(start + self.block_size, seq_len)
        self._local_valid_len = max(0, end - start)
        if self._local_valid_len > 0:
            shard = x.narrow(1, start, self._local_valid_len)
        else:
            shp = list(x.shape)
            shp[1] = 0
            shard = torch.empty(*shp, dtype=x.dtype, device=x.device)
        padded = self._pad_to_block_size(shard, dim=1)
        assert padded.size(1) == self.block_size and padded.device == x.device
        return padded
    ...
    def gather_tensor(
        self, tensor: Tensor, dim: int = 1
    ) -> Tensor:
        if self.world_size == 1:
            return tensor
        assert tensor.size(dim) == self.block_size
        t = tensor.contiguous()
