...
class RingAttentionHelper:
    ...
    def forward_full(self, q_local, k_local, v_local, mask_global, valid_len, x_block, x_norm_block, q_start_global):
        ...
        max_score = self._compute_max_score_pass(
            q_local, k_local, mask_global, q_start_global, T_q_local, debug_info=None
        )

        numerator, denominator = self._compute_sums_pass(
            q_local, k_local, v_local, mask_global, q_start_global, T_q_local, max_score, debug_info=None
        )
    ...
    def _compute_max_score_pass(self, q_local, k_local, mask_global, q_start_global, valid_len_local, debug_info):
        ...
        # Suggestion 5: Temporarily cast QKV to float32 for this pass
        ...
        current_k_block = k_local_f32 # Use float32 version
        ...
        for i in range(self.world_size):
            ...
            # Ensure current_mask is sliced correctly even if k_len is 0
            ...
            scores = self._compute_attention_scores(
                ...
                q_indices_global, k_indices_global, current_mask, apply_mask=True, keep_causal=True
            )
    ...
    def _compute_sums_pass(self, q_local, k_local, v_local, mask_global, q_start_global,
        ...
        current_k_block = k_local_f32 # Use float32 version
        current_v_block = v_local_f32 # Use float32 version
        current_k_len = k_local_f32.shape[2]
    ...
    def _compute_attention_scores(self, q, k, q_indices_global, k_indices_global, mask=None,
        ...
        scores = scores.clamp(min=-30.0, max=30.0) # Suggestion 3: Prevent softmax explosion
    ...
    def _ring_shift_tensor(self, tensor, pad_len):
        ...
        assert padded.shape[-2] == pad_len, f"[Rank {self.rank}] Padded shape mismatch: got {padded.shape[-2]}, expected {pad_len}" # Suggestion 6
        ...
        # Suggestion 1: Assert and Log Tensor Shapes During Ring Shifts
